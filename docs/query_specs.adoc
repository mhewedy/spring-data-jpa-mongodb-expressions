...
NOTE: Later in this section we will learn more about the `Expressions` API. Also you can see the <<#_public_api,Public API>>
for details on how to use the `Expressions` and `Expression` objects.
...
----
NOTE: Sometimes the join capabilities is not enough or the not best way for the use case you trying to implement,
