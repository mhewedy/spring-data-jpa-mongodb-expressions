= Introduction
include::include.adoc[]

*Spring Data JPA MongoDB Expressions* is a library that allows you to query Spring Data JPA Repositories using
https://docs.mongodb.com/manual/tutorial/query-documents/[MongoDB Query Language,role=external,window=_blank]. +
It parses (a subset of) MongoDB expressions and convert them to Spring Data JPA Specifications to be used with __Spring Data JPA__ Repositories. +
It is specially useful for so many JavaScript clients that like to build queries using convenient MongoDB expressions and
send the query to the Spring-based API to be processed by the Repository.

So, The idea is that you can build the _MongoDB_ query-like _JSON_ from the frontend app and pass it to the controller,
and then optionally you enrich it with additional conditions and pass it to the repository layer,
in which the _MongoDB_ query will be translated automatically to JPA specification and executed.
